...
export function getStudent(): StudentIdentity | null {
  try {
    ...
    if (!raw) return null;
    const v = JSON.parse(raw) as Partial<StudentIdentity>;
    if (typeof v.studentId !== 'string' || typeof v.number !== 'number') return null;
...
export async function resolveStudent(
  ...
  if (!supabaseUrl || !supabaseKey) return { ok: false, message: '接続先が設定されていません' };
  const code = joinCode.trim();
  if (!code) return { ok: false, message: 'がっきゅうコードを入れてください' };
  if (!Number.isInteger(number) || number < 1) return { ok: false, message: 'しゅっせき番号を入れてください' };
