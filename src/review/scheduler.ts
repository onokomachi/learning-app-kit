...
export const DEFAULT_INTERVALS_DAYS: readonly number[] = [1, 3, 7, 14, 30];
...
function intervalMs(box: number, intervals: readonly number[]): number {
...
export function getDueSkills(
  ...
  for (const [skillId, st] of Object.entries(states)) {
    if (!st || st.nextDueTs > now) continue;
