 ...
 */
import { getStudent, resolveStudent } from './student.js';
function urlEnv() {
    const g = globalThis;
...
export function buildHandoffUrl(appUrl, joinCode, number) {
 ...
 * - 受け取ったらURLから消す
 */
export async function adoptStudentFromUrl(config) {
    const url = urlEnv();
    if (!url)
        return getStudent();
    let joinCode = null;
