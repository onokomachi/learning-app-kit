export function checkStructure(r, a) {
    if (a.testSteps) {
        ...
        r.assert(missing.length === 0, `実装した ${a.skillIds.length} レベルすべてが出題プールに入っている`, `本番テストに一度も出ないレベルがある: ${missing.join(', ')}`);
        ...
        const subs = a.testSteps.map((s) => s.sub).filter((s) => !!s);
        r.assert(new Set(subs).size === subs.length, `設問 ${a.testSteps.length} 問（丸数字に重複なし）`, '丸数字が重複している');
    }
    ...
    r.assert(notDefined.length === 0, '実装した全レベルが モジュールのレベル一覧に出ている', `画面に出ないレベル: ${notDefined.join(', ')}`);
    let mismatch = 0;
